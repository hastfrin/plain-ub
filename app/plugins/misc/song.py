...
def is_yt_url(url: str) -> bool:
    return urlparse(url).netloc in dominios
...
async def song_dl(bot: BOT, message: Message) -> None | Message:
    """
    CMD: SONG
    INFO: Baixa uma música pelo nome ou link do YouTube e envia como áudio.
    USO: .song [nome da música ou link do YouTube]
    """
    query = extract_link_from_reply(message.replied) or message.filtered_input

    if not query:
        await message.reply("Informe o nome ou link da música para baixar.")
        return

    response: Message = await message.reply("Procurando...")
    ...
    if not audio_files:
        await response.edit("Música não encontrada.")
        return
    ...
    await response.edit(f"`Enviando {audio_file.name}...`")
...
async def get_download_info(query: str, path: Path) -> dict:
    """
    Executa o yt-dlp para baixar a música e retorna informações sobre o arquivo baixado.
    """
    download_cmd = (
