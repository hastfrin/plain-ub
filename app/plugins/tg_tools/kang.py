...
async def video_kang(message: Message, ff=False) -> tuple[str, None]:
    ...
    if video.file_size > 5242880:
        raise MemoryError("O arquivo excede o limite de 5MB.")
...
async def sticker_kang(message: Message, **_) -> tuple[str, str]:
    ...
    if sticker.is_animated:
        raise TypeError("Stickers animados não são suportados.")
...
async def get_sticker_set(
    ...
    if extra_config.CUSTOM_PACK_NAME:
        ...
        pack_title = f"Pacote do {user.username or core_utils.get_name(user)} - Vol {count}"
...
async def kang(bot: BOT, message: Message):
    """
    CMD: KANG
    INFO: Salva um sticker/imagem/gif/vídeo no seu pack de figurinhas.
    FLAGS: -f para acelerar vídeo para caber em 3 segundos.
    USO: .kang | .kang -f

    Diferenças da versão antiga:
        • Muito mais rápido pois usa métodos internos.
        • Usuários sudo têm seus próprios packs.
        • Se em modo duplo, o pack pertence ao respectivo sudo.
        • Junta stickers de foto e vídeo no mesmo pack.
        • Stickers de vídeo não limitados ao limite de 50 do Telegram.

    Obs: se quiser usar o método antigo, defina USE_LEGACY_KANG=1
    """
    ...
    if not media_func:
        await message.reply("<code>Mídia não suportada...</code>")
        return

    response = await message.reply("<code>Processando...</code>")
    ...
    try:
        ...
        await response.edit(
            f"Kang feito! <a href='t.me/addstickers/{stickers.set.short_name}'>Clique aqui para ver</a>",
