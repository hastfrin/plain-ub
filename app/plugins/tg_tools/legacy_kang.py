...
async def get_sticker_set(limit: int, is_video=False) -> tuple[str, str, bool]:
    ...
    if cus_nick := os.environ.get("CUSTOM_PACK_NAME"):
        ...
        pack_title = (
            f"{bot.me.username or core_utils.get_name(bot.me)} - pack{video} kang vol {count}"
        )
    return pack_title, f"{pack_name}{video}_{count}", criar_novo
...
async def video_kang(message: Message, ff=False) -> dict:
    ...
    if video.file_size > 5242880:
        raise MemoryError("Arquivo excede o limite de 5MB.")
...
async def kang_sticker(bot: BOT, message: Message):
    """
    CMD: LEGACY KANG
    INFO: Salva uma figurinha/imagem/gif/vídeo no seu pack.
    FLAGS: -f para acelerar vídeo e caber em 3 segundos.
    USO: .kang | .kang -f
    """
    ...
    if not media_func:
        await message.reply("Mídia não suportada.")
        return

    response: Message = await message.reply("<code>Processando...</code>")

    kwargs: dict = await media_func(message=replied, ff="-f" in message.flags)

    pack_title, pack_name, criar_novo = await get_sticker_set(
    ...
    if criar_novo:
        ...
        await response.edit(text=f"Kang feito: <a href='t.me/addstickers/{pack_name}'>clique aqui</a>")
        return
    ...
    await response.edit(
        text=f"Kang feito: <a href='t.me/addstickers/{pack_name}'>clique aqui</a>", disable_preview=True
    )
